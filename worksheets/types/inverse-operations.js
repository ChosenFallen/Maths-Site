...
function generateProblem(rand, difficulty) {
    if (difficulty === "easy") {
        ...
        if (type === 0) {
            ...
            const question = `${a} + ? = ${c}`;
            const answer = `${b}`;
            return { question, answer, answerHtml: answer, wrongAnswers: generateNumericDistracters(b, rand).map(wa => `${wa}`) };
        } else {
        ...
        if (type === 0) {
            ...
            const question = `? × ${b} = ${c}`;
            const answer = `${a}`;
            return { question, answer, answerHtml: answer, wrongAnswers: generateNumericDistracters(a, rand).map(wa => `${wa}`) };
        } else if (type === 1) {
            // Division: c ÷ b = ?, find ?
